...
The Docker build expects the generated backend model files to exist locally in
`trained_models/` before the image is built. Those files are deliberately not
tracked on `main` because they are generated model artifacts.
